...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        # (hidden.size() - 1x5x8, encoder_outputs.size() - 10x5x8)
        max_len = encoder_outputs.size(0)
        this_batch_size = encoder_outputs.size(1)

        # Create variable to store attention energies
        attn_energies = torch.zeros(this_batch_size, max_len) # B x Seq_len
...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs):
        ...
        embedded = embedded.view(1, batch_size, self.hidden_size) # S = 1 x B x hidden_size
        ...
        concat_output = F.tanh(self.concat(concat_input)) # B x hidden_size
